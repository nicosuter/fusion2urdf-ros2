...
def make_joints_dict(root, msg):
    ...
    for joint in root.joints:
        ...
        # Skip fully dangling ghost joints that could not be deleted in the
        # pre-pass (e.g. they live inside read-only derived components). They
        # reference no components, so they contribute nothing to the URDF.
        if joint.occurrenceOne is None and joint.occurrenceTwo is None:
            continue

        # Guard: a joint with exactly one missing side is attached to the root
        # component or to ground - the user must fix that one by hand.
        if joint.occurrenceOne is None or joint.occurrenceTwo is None:
            side = 'occurrenceOne (child)' if joint.occurrenceOne is None \
                else 'occurrenceTwo (parent)'
            good = joint.occurrenceTwo if joint.occurrenceOne is None \
                else joint.occurrenceOne
            good_path = good.fullPathName if good is not None else '?'
            # Select the offending joint so it highlights in the browser tree.
            try:
                ...
                pass
            msg = ("Joint '" + joint.name + "' has " + side + " connected to "
                   "the root component or to ground. The other side is "
                   "attached to: " + good_path + ". Both sides of every joint "
                   "must connect to sub-components. The joint is SELECTED in "
                   "the browser - edit or delete it, then try again.")
            break
